...
echo Starting KYC Service (Port 8003)...
start "KYC Service" cmd /k "cd kyc_backend && python -m uvicorn app.main:app --port 8003 --reload"
timeout /t 3 /nobreak
...
echo Starting Translation + Groq Service (Port 8002)...
start "Translation Service" cmd /k "cd translation_backend && python -m uvicorn app.main:app --port 8002 --reload"
timeout /t 3 /nobreak
...
echo Starting Pipeline Orchestrator (Port 8004)...
start "Pipeline Service" cmd /k "cd backend && python -m uvicorn pipeline:app --port 8004 --reload"
timeout /t 3 /nobreak
